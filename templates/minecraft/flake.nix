{
  ...
  #
  # Tailscale command (while in the minecraft directory and after running nix develop .#server):
    ...
    in
    {
      ...
      apps = nixpkgs.lib.genAttrs systems (
        system: {
          server = {
            type = "app";
            program = "${self.packages.${system}.server}/bin/start-server";
          };
          ...
          prism = {
            type = "app";
            program = "${self.packages.${system}.prism}/bin/launch-prism";
          };
        }
      );
    };
