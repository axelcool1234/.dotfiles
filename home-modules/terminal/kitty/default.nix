...
let
  ...
  kittyProvider = theme.providerFor program;
  themeFonts = theme.requireThemeData "fonts";
  themeIncludeLine = "include ~/.config/dotfiles-theme/kitty.conf\n";
  kittyFontConfigText =
    let
      postscriptSuffix =
  ...
  kittyConfigText =
    ...
    + lib.replaceStrings
      [ themeIncludeLine ]
      [ (if theme.isHandledByStylix kittyProvider then "" else themeIncludeLine) ]
      text;
in
{
