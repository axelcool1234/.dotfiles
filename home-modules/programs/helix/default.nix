...
in
{
  ...
  config = mkIf program-module.enable {
    xdg.configFile = lib.optionalAttrs (theme.providerIsAsset program) {
      "helix/themes/${theme.requireProviderOption program "themeName"}.toml".source = theme.requireAssetSource program;
    };
    programs.${program} = {
      enable = true;
      defaultEditor = true;
      ...
      settings = lib.optionalAttrs (theme.lookupProviderOption program "themeName" != null) {
        theme = theme.requireProviderOption program "themeName";
      } // {
        editor = {
          scrolloff = 8;
          auto-pairs = false;
