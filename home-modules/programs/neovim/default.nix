...
let
  ...
  # Neovim only needs a theme plugin when the selected provider is structured
  # and points at a vimPlugins attrPath for the colorscheme package.
  neovimThemePlugin = theme.matchProvider program {
    null = null;
    structured = provider:
      if provider.attrPath == [ ] then
        null
      else
        builtins.foldl' (acc: name: builtins.getAttr name acc) pkgs.vimPlugins provider.attrPath;
    default = _: null;
  };
