...
function M.new(opts)
  ...
  local function textobject_paragraph_entry(source_entry, around)
    ...
    local function paragraph_rows_from_row(target_row)
      local start_row = paragraph_start_at_or_before(target_row)
      local end_row = paragraph_end_at_or_after(target_row)
      return start_row, end_row
    end

    local function extend_paragraphs_forward(start_row, end_row)
      local paragraphs = 1
      local search_row = end_row + 1

      while paragraphs < count and search_row <= last_row do
        while search_row <= last_row and line_is_blank(search_row) do
          search_row = search_row + 1
        end

        if search_row > last_row then
          break
        end

        end_row = paragraph_end_at_or_after(search_row)
    ...
    if line_is_blank(row) then
      ...
      if next_row and not line_is_blank(next_row) then
        local start_row, end_row = paragraph_rows_from_row(next_row)
        start_row, end_row = extend_paragraphs_forward(start_row, end_row)
        return paragraph_entry(start_row, end_row, around)
      end

      if prev_row and not line_is_blank(prev_row) then
        local start_row, end_row = paragraph_rows_from_row(prev_row)
        start_row, end_row = extend_paragraphs_forward(start_row, end_row)
        return paragraph_entry(start_row, end_row, around)
      end
  ...
  pair_region_from_ts_node = function(node, char)
    ...
    return {
      start_pos = char_pos_from_byte_col0(start_row + 1, start_col),
      end_pos = position.prev_pos(current_buffer(), char_pos_from_byte_col0(end_row + 1, end_col)),
      open_char = open_char,
      close_char = close_char,
    }
  end
  ...
  function match.surround_add()
    ...
    if addition == "\r" or addition == "\n" then
      local line_ending = vim.bo[current_buffer()].fileformat == "dos" and "\r\n" or "\n"
      addition = line_ending
    end
