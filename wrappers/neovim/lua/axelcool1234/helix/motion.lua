...
local function stepped_pos(buffer, start_pos, forward, count)
  local pos = { start_pos[1], start_pos[2] }

  for _ = 1, count do
    local next_pos = step_pos(buffer, pos, forward)
      ...
      while true do
        ...
        if range_start[2] ~= current_pos[2] then
          ...
          if current_type == character_types.EndOfLine
            ...
            local previous_pos = position.prev_pos(buffer, current_pos)
            if pos_equal(previous_pos, current_pos) then
              break
            end
