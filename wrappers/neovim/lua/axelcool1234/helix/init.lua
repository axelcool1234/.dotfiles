...
local function visual_column_at_pos(pos)
  local line = line_text(pos[1])
  local prefix_end = math.min(math.max(pos[2] - 1, 0), #line)
  return vim.fn.strdisplaywidth(line:sub(1, prefix_end))
end
...
function M.goto_file_targets()
  ...
  local function best_span_target(span_text, cursor_col)
    ...
    for start_col = 1, #span_text do
      if start_col <= cursor_col + 1 then
        for end_col = start_col, #span_text do
          if cursor_col <= end_col + 1 then
            local text = span_text:sub(start_col, end_col)
  ...
  local function detect_target_near_cursor(entry)
    ...
    if span_start then
      spans[#spans + 1] = { start_col = span_start, end_col = #line }
    end

    for _, span in ipairs(spans) do
      if span.start_col <= cursor_col + 1 and cursor_col <= span.end_col + 1 then
        local span_text = line:sub(span.start_col, span.end_col)
