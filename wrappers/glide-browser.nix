...
let
  # Noctalia v5 speaks the Pywalfox native-messaging protocol directly.
  usePywalfox = hostVars.desktopShell == "noctalia";

  pywalfoxManifest = pkgs.writeText "pywalfox.json" ''
    {
      "name": "pywalfox",
      "description": "Apply Noctalia's current theme to Pywalfox",
      "path": "${lib.getExe selfPkgs.noctalia}",
      "type": "stdio",
...
in
{
  ...
  config = {
    ...
    runShell = [
      ...
      ''if [ ! -e "$GLIDE_PROFILE_DIR/glide/glide.ts" ] || ! cmp -s ${config.constructFiles.glideTs.path} "$GLIDE_PROFILE_DIR/glide/glide.ts"; then rm -f "$GLIDE_PROFILE_DIR/glide/glide.ts"; install -m 0644 ${config.constructFiles.glideTs.path} "$GLIDE_PROFILE_DIR/glide/glide.ts"; fi''
    ] ++ lib.optionals usePywalfox [
      ''mkdir -p "${"$"}{HOME}/.glide-browser/native-messaging-hosts"''
      ''rm -f "${"$"}{HOME}/.glide-browser/native-messaging-hosts/pywalfox.json"; install -m 0644 ${pywalfoxManifest} "${"$"}{HOME}/.glide-browser/native-messaging-hosts/pywalfox.json"''
    ];
    ...
    constructFiles.glideTs = {
      ...
      content = /* typescript */ ''
        ...
        // - `C-n` and `C-p` cycle through native urlbar suggestions.
        // - `Esc` leaves the native urlbar via Firefox's `F6` focus cycle and returns to normal mode.
        ...
        // Track how the next native urlbar Enter press should behave after `t` / `T`.
        let urlbarEnterDisposition = null;
        const isUrlbarFocused = () => document.activeElement?.id === "urlbar-input";
        const safeIsEditing = async () => {
          try {
            return await glide.ctx.is_editing();
          } catch (_error) {
            return false;
          }
        };
        ...
        // Esc closes the findbar, uses Firefox's native F6 cycle to leave the
        // urlbar, and otherwise just returns to normal mode without forcing
        // page textboxes to lose focus.
