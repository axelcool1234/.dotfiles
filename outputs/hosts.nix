{ lib, myLib, ... }:
let
  hostDirs = myLib.importTree.dirsWithFile ../hosts "configuration.nix";
in
# Convert each host directory into a reusable module value under `self.hosts`.
lib.pipe hostDirs [
  # Step 1: keep only the host names.
  # Output shape:
  # [ "fermi" "legion" "vm" ]
  lib.attrNames

  # Step 2: turn each host name into the `{ name, value }` shape expected by
  ...
  # Step 3: convert the list of records into the final attrset.
  # Final output shape:
  # {
